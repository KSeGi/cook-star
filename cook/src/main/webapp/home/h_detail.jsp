...
<head>
	...
	<script src="http://code.jquery.com/jquery-3.6.0.min.js"></script>
		...
		<!--  메인 -->
				<div id="detail">
					<c:forEach items="${HomeDetail}" var = "detail">
					
			        	<c:choose>
				        	<c:when test = "${sessionScope.user.u_grade==2}">
								<a href="HomeDelete?h_no=${detail.getH_no()}" class="btn">삭제</a>
				        		<a href="HomeUpdate?h_no=${detail.getH_no()}" class="btn">수정</a>
							</c:when>
							<c:when test = "${sessionScope.user.u_id==detail.getH_id()}">
								<a href="HomeDelete?h_no=${detail.getH_no()}" class="btn">삭제</a>
								<a href="HomeUpdate?h_no=${detail.getH_no()}" class="btn">수정</a>
							</c:when>
						</c:choose>
						...
						<ul>							
							<c:if test="${detail.getH_list()==1}"><li class="list">다인요리</li></c:if>
							<c:if test="${detail.getH_list()==2}"><li class="list">1인요리</li></c:if>
							<c:if test="${detail.getH_list()==3}"><li class="list">채식요리</li></c:if>
							<li class="title">${detail.getH_title()}</li>
						</ul>
						<ul class="info">
							<c:if test="${sessionScope.user.u_id!=null }">
								<c:choose>
									...
									<c:otherwise>
										<li class="wish"><a href ="" onclick="wish_like(${detail.getH_no()})">☆</a></li>
									</c:otherwise>
								</c:choose>
							</c:if>
							<li class="nik">${detail.getNik()}</li>
							<li class="hit">${detail.getH_hit()}</li>
							<li class="date">${detail.getH_joindate()}</li>
						</ul>
						<ul class="content">
							<c:if test ="${detail.getH_img() ne '/files/null' and detail.getH_img()!=null and detail.getH_img()!=''}">
							<li class="img"><img src="./${detail.getH_img()}"></li>
							</c:if>
							<li class="star">${detail.getH_star()}</li>
							<li class="text">
								<textarea readonly>${detail.getH_text()}</textarea>
							</li>
						</ul>
					</c:forEach>
				</div>
		...
		<!--  댓글 작성창  -->
				<div>
					...
					<input type="submit" class ="cbt" value="작성" onclick = "insertCo(${home.h_no})">
				</div>
		<!--  댓글 목록/수정창 -->
