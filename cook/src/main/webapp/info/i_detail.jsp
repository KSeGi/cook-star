...
<html>
	<head>
		...
		<!--  메인 -->
				<div id="detail">
					<c:forEach items="${InfoDetail}" var = "detail">
					
			        	
				        <c:if test = "${sessionScope.user.u_grade==2}">
							<a href="InfoDelete?i_no=${detail.getI_no()}" class="btn">삭제</a>
				        	<a href="InfoUpdate?i_no=${detail.getI_no()}" class="btn">수정</a>
						</c:if>
					
						<ul>							
							<li class="list">공지사항</li>
							<li class="title">${detail.getI_title()}</li>
						</ul>
						<ul class="info">
							<li class="nik">${detail.getI_nik()}</li>
							<li class="hit">${detail.getI_hit()}</li>
							<li class="date">${detail.getI_joindate()}</li>
						</ul>
						<ul class="content">
						<c:if test ="${detail.getI_img() ne '/files/null' and detail.getI_img()!=null and detail.getI_img()!=''}">
							<li class="img"><img src="./${detail.getI_img()}"></li>
						</c:if>
							<li class="text">
								<textarea readonly>${detail.getI_text()}</textarea>
							</li>
						</ul>
					</c:forEach>
				</div>
		...
		<!--  댓글 작성창  -->
				<div>
					...
					<input type="submit" class ="cbt" value="작성" onclick = "insertCo(${info.i_no})">
				</div>
		<!--  댓글 목록/수정창 -->
				<c:forEach items="${Comment}" var="co" varStatus="i">
					...
					<table>
						<tr>
							...
							<td <c:if test="${sessionScope.user.u_grade==2}"> colspan ="2"</c:if> 
