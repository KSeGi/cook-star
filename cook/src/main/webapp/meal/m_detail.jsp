...
<html>
	<head>
		...
		<!--  메인 -->
				<div id="detail">
					<c:forEach items="${MealDetail}" var = "detail">
					
			        	<c:choose>
				        	<c:when test = "${sessionScope.user.u_grade==2}">
								<a href="MealDelete?m_no=${detail.getM_no()}" class="btn">삭제</a>
				        		<a href="MealUpdate?m_no=${detail.getM_no()}" class="btn">수정</a>
							</c:when>
							<c:when test = "${sessionScope.user.u_id==detail.getM_id()}">
								<a href="MealDelete?m_no=${detail.getM_no()}" class="btn">삭제</a>
								<a href="MealUpdate?m_no=${detail.getM_no()}" class="btn">수정</a>
							</c:when>
						</c:choose>
						...
						<ul>							
							...
							<li class="title">${detail.getM_title()}</li>
						</ul>
						<ul class="info">
							<c:if test="${sessionScope.user.u_id!=null }">
								<c:choose>
									...
									<c:otherwise>
										<li class="wish"><a href ="" onclick="wish_like(${detail.getM_no()})">☆</a></li>
									</c:otherwise>
								</c:choose>
							</c:if>
							<li class="nik">${detail.getM_nik()}</li>
							<li class="hit">${detail.getM_hit()}</li>
							<li class="date">${detail.getM_joindate()}</li>
						</ul>
						<ul class="content">
							<c:if test ="${detail.getM_img() ne '/files/null' and detail.getM_img()!=null and detail.getM_img()!=''}">
							<li class="img"><img src="./${detail.getM_img()}"></li>
							</c:if>
							<li class="star">${detail.getM_star()}</li>
							<li class="text">
								<textarea readonly>${detail.getM_text()}</textarea>
							</li>
						</ul>
					</c:forEach>
				</div>
		...
		<!--  댓글 작성창  -->
				<div>
					...
					<input type="submit" class ="cbt" value="작성" onclick = "insertCo(${meal.m_no})">
				</div>
		<!--  댓글 목록/수정창 -->
