...
public class Myboard extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		List<Home> hlist = hdao.selectlist(user.getU_no());
		request.setAttribute("HomeList", hlist);
		
		GeubDAO gdao = GeubDAO.getInstance();
		List<Geub> glist = gdao.selectlist(user.getU_no());
		request.setAttribute("GeubList", glist);
		
		MealDAO mdao = MealDAO.getInstance();
		List<Meal> mlist = mdao.selectlist(user.getU_no());
		request.setAttribute("MealList", mlist);
		
		RequestDispatcher dis = request.getRequestDispatcher("mypage/myboard.jsp");
